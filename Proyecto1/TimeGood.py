...
i = 0
Tm = int(input("Cual es tu zona horaria en GMT:")) #pedirte la Zona horaria
while True:
    ...
    if yearsCE % 4 == 0 and (not yearsCE % 100 == 0 or yearsCE % 400 == 0): # Si el año es bisiesto, hacer que febrero tenga 29 dias
        months[1] = 29
    
    days = ((t//86400) % 365) - (yearsUnix-2)//4 #Obtener dia a partir de la hora unix, asegurarse de que no sea mayor que 365 dias, y hacer que no sea siempre cuatro
    i = 0
    ...
    print("Year" ,int(yearsCE), "/",month ,"month /", int(days), "days /", int(hours), "hours /", int(mins), "mins /", int(secs), "secs", end="\r") # Imprimir los numeros
